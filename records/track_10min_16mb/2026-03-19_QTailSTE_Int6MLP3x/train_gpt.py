...
class Hyperparameters:
    ...
    init_state_path = os.environ.get("INIT_STATE_PATH", "")
    ...
    qtail_start_frac = float(os.environ.get("QTAIL_START_FRAC", 0.82))
...
FP16_TAIL_K_LAYERS = int(os.environ.get("FP16_TAIL_K_LAYERS", 2))
FP16_TAIL_V_LAYERS = int(os.environ.get("FP16_TAIL_V_LAYERS", 0))
FP16_TAIL_O_LAYERS = int(os.environ.get("FP16_TAIL_O_LAYERS", 0))
FP16_EXTRA_WEIGHT_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get("FP16_EXTRA_WEIGHT_PATTERNS", "").split(",")
    if pattern
)
...
def get_num_layers_total_from_names(names) -> int:
    return max((int(name.split(".")[1]) for name in names if name.startswith("blocks.")), default=-1) + 1
...
def matches_tail_weight(name: str, suffix: str, tail_layers: int, num_layers_total: int) -> bool:
    if tail_layers <= 0 or not name.endswith(suffix):
        return False
    block_idx = get_block_index(name)
    if block_idx is None or num_layers_total <= 0:
        return False
    return block_idx >= num_layers_total - tail_layers

def should_keep_weight_fp16(name: str, num_layers_total: int) -> bool:
    if name == "tok_emb.weight":
        return True
    if matches_tail_weight(name, "attn.c_k.weight", FP16_TAIL_K_LAYERS, num_layers_total):
        return True
    if matches_tail_weight(name, "attn.c_v.weight", FP16_TAIL_V_LAYERS, num_layers_total):
        return True
    if matches_tail_weight(name, "attn.proj.weight", FP16_TAIL_O_LAYERS, num_layers_total):
        return True
    return any(pattern in name for pattern in FP16_EXTRA_WEIGHT_PATTERNS)
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    num_layers_total = get_num_layers_total_from_names(state_dict)
    ...
    for name, tensor in state_dict.items():
        ...
        if should_keep_weight_fp16(name, num_layers_total):
...
def fake_quantize_weight_ste(t: Tensor, bits: int = 6) -> Tensor:
    if t.ndim != 2:
        return t
    max_val = (2 ** (bits - 1)) - 1
    t32 = t.float()
    clip_abs = torch.quantile(t32.abs(), INT8_CLIP_Q, dim=1).clamp_min(1.0 / float(max_val))
    clipped = torch.maximum(torch.minimum(t32, clip_abs[:, None]), -clip_abs[:, None])
    scale = (clip_abs / float(max_val)).clamp_min(1.0 / float(max_val))
    q = torch.clamp(torch.round(clipped / scale[:, None]), -max_val, max_val)
    dq = q * scale[:, None]
    return t32 + (dq - t32).detach()
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        weight = self.weight
        if self.training and getattr(self, "qtail_ste_active", False) and getattr(self, "qtail_ste_eligible", True):
            weight = fake_quantize_weight_ste(weight, bits=6)
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, weight.to(x.dtype), bias)

def configure_qtail_ste_modules(module: nn.Module, num_layers_total: int) -> int:
    eligible_count = 0
    for name, submodule in module.named_modules():
        if not isinstance(submodule, CastedLinear):
            continue
        weight_name = f"{name}.weight" if name else "weight"
        eligible = not should_keep_weight_fp16(weight_name, num_layers_total)
...
def main() -> None:
    ...
    train_token_den = world_size * grad_accum_steps
    if args.train_batch_tokens % train_token_den != 0:
        raise ValueError(
            "TRAIN_BATCH_TOKENS must divide evenly across data-parallel workers and grad accumulation; "
            f"got TRAIN_BATCH_TOKENS={args.train_batch_tokens}, WORLD_SIZE={world_size}, "
            f"GRAD_ACCUM_STEPS={grad_accum_steps}"
        )
    local_train_tokens = args.train_batch_tokens // train_token_den
    if local_train_tokens % args.train_seq_len != 0:
        raise ValueError(
            "Per-rank train tokens must be a multiple of TRAIN_SEQ_LEN; "
    ...
    if args.init_state_path:
        init_state = torch.load(args.init_state_path, map_location="cpu")
        base_model.load_state_dict(init_state, strict=True)
        log0(f"init_state:loaded path={args.init_state_path}")
    qtail_eligible_modules = configure_qtail_ste_modules(base_model, args.num_layers)
